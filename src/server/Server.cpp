	...
	while ((pos = buf.find("\r\n")) != std::string::npos)
	{
		...
		if (!split_msg.empty())
		{
			...
			if (!client.getIsRegistered() && isCommandRequiresRegistration(split_msg[0]))
				sendMessage(client.getFd(), ":" + _serverName + " " + ERR_NOTREGISTERED + " " + (client.getNickname().empty() ? "*" : client.getNickname()) + " :You have not registered");
			else if (it != _cmdMap.end())
				(this->*(it->second))(client, message, split_msg);
			else
				sendMessage(client.getFd(), ":" + _serverName + " " + ERR_UNKNOWNCOMMAND + " " + (client.getNickname().empty() ? "*" : client.getNickname()) + " " + split_msg[0] + " :Unknown command");
		}
...
void Server::handlePart(Client &client, const std::string &rawMsg, const std::vector<std::string> &tokens)
{
	...
	{
		broadcastToChannel(chanIt->second.getName(), ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHost() + " PART " + chanIt->second.getName() + ((tokens.size() > 2) ? (" :" + tokens[2]) : ""), 0);
...
void Server::handleTopic(Client &client, const std::string &rawMsg, const std::vector<std::string> &tokens)
{
	...
	if (res.compare(ERR_NOTONCHANNEL) == 0 || res.compare(ERR_CHANOPRIVSNEEDED) == 0)
		...
		sendMessage(client.getFd(), ":" + _serverName + " " + res + " " + client.getNickname() + " " + chanIt->second.getName() + " :" + chanIt->second.getTopic());
	else if (res.compare(RPL_NOTOPIC) == 0)
		sendMessage(client.getFd(), ":" + _serverName + " " + res + " " + client.getNickname() + " " + chanIt->second.getName() + " :No topic is set");
	else
		broadcastToChannel(chanIt->second.getName(), ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHost() + " TOPIC " + chanIt->second.getName() + " :" + chanIt->second.getTopic(), client.getFd());
}
...
void Server::handlePrivmsg(Client &client, const std::string &rawMsg, const std::vector<std::string> &tokens)
{
	...
	if (tokens[1].at(0) == '#')
	{
		...
		broadcastToChannel(chanIt->second.getName(), ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHost() + " PRIVMSG " + chanIt->second.getName() + " :" + tokens[2], client.getFd());
	}
	else
	{
		targetIt = findClientByNick(_clients, tokens[1]);
		...
		sendMessage(targetIt->second.getFd(), ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHost() + " PRIVMSG " + targetIt->second.getNickname() + " :" + tokens[2]);
	}
}
...
void Server::handleKick(Client &client, const std::string &rawMsg, const std::vector<std::string> &tokens)
{
	...
	broadcastToChannel(chanIt->second.getName(), ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHost() + " KICK " + chanIt->second.getName() + " " + targetIt->second.getNickname() + ((tokens.size() > 3) ? (" :" + tokens[3]) : ""), 0);
